<template>
  <div class="seat-container" v-loading="loading">
    <el-card class="box-card">
      ...
      >
        <el-form-item label="放映厅">
          <el-input v-model="searchParams.likeHallName" />
        </el-form-item>
    ...
    <el-card class="box-card" body-style>
      ...
      <el-table :data="list" border style="width: 100%">
        <el-table-column prop="hallName" label="放映厅" width="180" />
        <el-table-column prop="row" label="排" />
        <el-table-column prop="col" label="座" />
...
export default {
  ...
  computed: {
    ...mapState({
      seatList: state => state.seat.list,
      pagination: state => state.seat.pagination
    })
  },
  created() {
    this.getDataList();
  ...
  methods: {
    getDataList() {
      this.loading = true;
      this.$store.dispatch("seat/list", this.searchParams).then(() => {
        this.list = this.seatList;
    ...
    handleDelete(index, row) {
      this.$confirm("确定删除数据?", "提示", {
        ...
        this.$store.dispatch("seat/deleteById", row.id).then(() => {
          this.$message({
            message: "删除成功",
            type: "success"
          });
