  ...
  <div class="orders-container">
    ...
    <el-card class="box-card" body-style>
      ...
      <el-table :data="list" border style="width: 100%">
        ...
        <el-table-column prop="sessionStartTime" label="场次开始时间" />
...
export default {
  ...
  methods: {
    getDataList() {
      this.$store.dispatch("orders/list", this.searchParams).then(() => {
    ...
    handleDelete(index, row) {
      this.$confirm("确定删除数据?", "提示", {
        ...
        this.$store.dispatch("orders/deleteById", row.id).then(() => {
          this.$message({
            message: "删除成功",
            type: "success"
          });
